...
- `tests/fwu_policy.test.js` — the decision engine. Pure, no hardware, no network.
- `tests/fwu_http.test.js` — the download layer, against a local HTTPS server
  standing in for the GitHub Releases API: the token is dropped on the redirect
  to the object store, a truncated or wrong-digest image is refused and leaves
  the cache clean, and a verified image is reused without a second fetch. These
  need a throwaway certificate, generated by `scripts/gen-test-cert.js`; without
  `openssl` they skip rather than fail.
